...
// Embed UnimplementedByzantineServer to satisfy the interface.
type General struct {
	pb.UnimplementedByzantineServer
	id            int
	address       string
	isCommander   bool
	isTraitor     bool
...
func (g *General) runRound(round int, peers []*General, wg *sync.WaitGroup) {
	...
	if round == 1 {
		if g.isCommander {
			// Commander sends its order to all lieutenants.
			log.Printf("General %d [Round %d] (Commander) sending order: %s", g.id, round, g.currentOrder)
			for _, peer := range peers {
				go sendMessage(g, peer, round, g.getMessageForPeer(peer))
			}
		} else {
			// Lieutenants: set up channel and wait for the commander's message.
