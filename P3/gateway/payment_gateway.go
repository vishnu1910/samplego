...
func (s *PaymentGatewayServer) RegisterClient(ctx context.Context, req *pb.RegisterRequest) (*pb.RegisterResponse, error) {
	...
	log.Printf("[REGISTER] Client %s registered with bank %s, account %s", clientID, req.Bank, req.AccountNumber)
...
func (s *PaymentGatewayServer) ProcessPayment(ctx context.Context, req *pb.PaymentRequest) (*pb.PaymentResponse, error) {
	// Check idempotency
	if res, exists := s.idempotencyMap.Load(req.IdempotencyKey); exists {
		log.Printf("[IDEMPOTENCY] Returning stored result for key: %s", req.IdempotencyKey)
	...
	// Create metadata with credentials for bank server calls.
	// Make sure your Bank Server interceptor accepts these credentials.
	md := metadata.Pairs("username", "gateway", "password", "gatewaypassword")
	ctxWithCreds := metadata.NewOutgoingContext(ctx2, md)

	// Call Prepare on both bank servers (sender and receiver)
	senderClient, ok := s.bankClients[req.FromBank]
	if !ok {
		return nil, errors.New("sender bank not found")
	}
	receiverClient, ok := s.bankClients[req.ToBank]
	if !ok {
	...
	go func() {
		resp, err := senderClient.Prepare(ctxWithCreds, senderPrepReq)
		if err != nil {
	...
	go func() {
		resp, err := receiverClient.Prepare(ctxWithCreds, receiverPrepReq)
		if err != nil {
	...
	for i := 0; i < 2; i++ {
		...
		case err := <-errChan:
			...
			s.abortTransaction(ctxWithCreds, transactionID, senderClient, receiverClient)
			s.storeIdempotency(req.IdempotencyKey, false, err.Error())
		...
		case <-ctx2.Done():
			...
			s.storeIdempotency(req.IdempotencyKey, false, msg)
			return &pb.PaymentResponse{
				Success: false,
				Message: msg,
			}, nil
	...
	commitReq := &pb.CommitRequest{TransactionId: transactionID}
	senderCommitChan := make(chan *pb.CommitResponse, 1)
	receiverCommitChan := make(chan *pb.CommitResponse, 1)
	go func() {
		resp, err := senderClient.Commit(ctxWithCreds, commitReq)
		if err != nil {
	...
	go func() {
		resp, err := receiverClient.Commit(ctxWithCreds, commitReq)
		if err != nil {
	...
	s.updateLocalBalance(req.ClientId, req.Amount)

	msg := fmt.Sprintf("Payment of %f successful. Transaction ID: %s", req.Amount, transactionID)
	s.storeIdempotency(req.IdempotencyKey, true, msg)
	return &pb.PaymentResponse{
		Success: true,
		Message: msg,
	}, nil
...
func (s *PaymentGatewayServer) abortTransaction(ctx context.Context, transactionID string, senderClient, receiverClient pb.BankServiceClient) {
	abortReq := &pb.AbortRequest{TransactionId: transactionID}
...
func (s *PaymentGatewayServer) GetBalance(ctx context.Context, req *pb.BalanceRequest) (*pb.BalanceResponse, error) {
	val, ok := s.accounts.Load(req.ClientId)
	if !ok {
